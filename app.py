...
st.markdown("""
<style>
:root { --ink:#17202a; --muted:#64748b; --brand:#7c3aed; --soft:#f6f4ff; }
.block-container {padding-top: 1rem; padding-bottom: 3rem; max-width: 1350px;}
[data-testid="stMetric"] {background:white; border:1px solid #eceff3; border-radius:16px; padding:14px; box-shadow:0 4px 18px rgba(15,23,42,.04);}
.hero {padding:24px; border-radius:24px; background:linear-gradient(135deg,#17132e,#4c1d95); color:white; margin-bottom:16px;}
.hero h1 {margin:0; font-size:2rem;}.hero p{margin:.4rem 0 0;opacity:.84}
.card {background:white; border:1px solid #e8ebf0; border-radius:18px; padding:18px; margin:8px 0; box-shadow:0 4px 18px rgba(15,23,42,.035);}
.verdict-good {background:#ecfdf5;border-left:6px solid #10b981;padding:18px;border-radius:14px;}
.verdict-warn {background:#fffbeb;border-left:6px solid #f59e0b;padding:18px;border-radius:14px;}
.verdict-bad {background:#fef2f2;border-left:6px solid #ef4444;padding:18px;border-radius:14px;}
...
@media (max-width: 700px){.hero{padding:18px}.hero h1{font-size:1.55rem}.block-container{padding-left:.7rem;padding-right:.7rem}}
</style>
...
if mode=="Listed Share":
    ...
    if run or symbol:
        ...
        verdict="Strong research candidate" if d['overall']>=70 else "Mixed — price and risks matter" if d['overall']>=50 else "High caution required"
        st.markdown(f"<div class='{verdict_class(d['overall'])}'><b>30-second verdict: {verdict}</b><br>{explain_pe(d['pe'])} The score is a structured checklist, not a buy/sell instruction.</div>",unsafe_allow_html=True)

        tab1,tab2,tab3,tab4,tab5,tab6=st.tabs(["⚡ 5-Min Dashboard","📈 Financials","🧬 Business DNA","💰 Valuation","⚠️ Risks","📄 PDF"])
        with tab1:
...
# ================= IPO =================
    ...
    st.info('The final production engine should auto-fill these from RHP/DRHP, exchange subscription data and a licensed market feed. They remain editable here so the analysis and UI can be tested now.')
    ...
    t1,t2,t3,t4,t5=st.tabs(['⚡ 5-Min Dashboard','📊 Graphs','🏢 Business & Dependency','⚠️ Risks','📄 PDF'])
    with t1:
